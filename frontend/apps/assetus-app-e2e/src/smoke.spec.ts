...
// crashing. Full "renders assets data" needs an authenticated session + seeded
...
test('app boots and redirects unauthenticated user to login', async ({
  ...
  await expect(page.locator('assetus-root')).toBeAttached();
...
test('space-scoped lists route loads without crashing', async ({ page }) => {
  ...
  // Lazy-loads the assetus space shell + assets route. Unauthenticated, this
  // redirects to login; the assertion is that the app handles it without throwing.
  await page.goto('/space/family/smoke-test-space/assets');
