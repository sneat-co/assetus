...
export const appRoutes: Route[] = [
  {
    ...
    pathMatch: 'full',
    loadComponent: () =>
      import('./home/assetus-home-page.component').then(
        (m) => m.AssetusHomePageComponent,
      ),
    canActivate: [AuthGuard],
    data: { authGuardPipe: () => redirectToLoginIfNotSignedIn },
  },
  {
    // Space-scoped routes host the assetus pages, mirroring sneat-app's
    // space/:spaceType/:spaceID mount point.
    path: 'space/:spaceType/:spaceID',
    loadChildren: () =>
      import('./space/assetus-space.routes').then((m) => m.assetusSpaceRoutes),
  },
  {
